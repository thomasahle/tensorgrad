...
\def\tn@leg#1/#2/#3\@nil{%
  \tn@checkstar#1*\@nil
  \iftn@d % derivative whisker: fans out from the SINGLE dot at north east
    \global\tn@dseentrue
    \draw[every leg] (\tikzlastnode.north east) -- ++(\tn@ang:\pgfkeysvalueof{/tikz/leg length});%
  \else
...
\begin{tikzpicture}[tn]
  \node[legs={180}] (a) {$a$};
  \node[right=.8em of a, legs={40*,20*}] (X) {$(X)$};
