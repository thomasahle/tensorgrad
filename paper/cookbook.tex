...
& \frac{\partial}{\partial \mathbf{X}} \operatorname{Tr}(\mathbf{X} \otimes \mathbf{X})=\frac{\partial}{\partial \mathbf{X}} \operatorname{Tr}(\mathbf{X}) \operatorname{Tr}(\mathbf{X})=2 \operatorname{Tr}(\mathbf{X}) \mathbf{I}(120) \\
&
\end{aligned}
...
\begin{tabular}[h]{lccc}
   ...
   & \begin{tikzpicture}[baseline=-.25em]
      ...
      \node (x) at (1,0) {$v$};
      \path (-.27, .25) edge [out=60, in=120, looseness=1] (x);
...
Then we can write soft-max as the tensor diagram:
\[
   \begin{tikzpicture}[baseline=-.25em]
      \node (ex) at (0,0) {$\{\mathrm{exp} \quad x\}$}
      \node[right=1em of ex] (p) {$\{\mathrm{pow}^{-1} \quad \{\mathrm{exp} \quad x\}\quad \sbullet\,\}$};
      \path ($(p.east)+(-1.1,.15)$) edge [out=90, in=90, looseness=1] ($(p.east)+(-.4,.15)$);
      \draw ($(ex) + (.5, .15)) -- ++(.1,.2);
   \end{tikzpicture}
   .
\]
