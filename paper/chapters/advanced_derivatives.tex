...
\begin{walign}
   ...
   \begin{tikzpicture}[baseline=(a0.base), inner sep=1pt]
      ...
      \draw (a0) -- (a1) -- (a2) -- (a3);
      \draw (a0.west) .. controls +(-.3,0) .. ++(-.5,.4);
      \draw (a3.east) .. controls +(.3,0) .. ++(.5,.4);
   \end{tikzpicture}
   \\
   \tag{64}
...
Penrose~\cite{penrose1971} read this identity the other way around,
as a diagrammatic formula for the inverse itself:
\[
   X^{-1}
   \;=\;
   \frac{n}{\detstack{$X$}}
   ...
   \mathbin{\begin{tikzpicture}[baseline=(a0.base), inner sep=1pt]
      ...
      \draw (a0.south) -- ++(0,-.2) coordinate (a0bot);
      \draw (a1.south) -- ++(0,-.2) coordinate (a1bot);
      \path (g.north) ++(0,.2) coordinate (gtop);
      \path (g.south) ++(0,-.2) coordinate (gbot);
      \draw[line width=2pt] (a0top -| a0.west) -- (gtop -| g.east);
      \draw[line width=2pt] (a0bot -| a0.west) -- (gbot -| g.east);
      \draw (gtop) .. controls +(0,-.4) and +(-.5,0) .. ($(g.east)+(1.1,-.12)$);
      \draw (gbot) .. controls +(0,.4) and +(-.5,0) .. ($(g.east)+(1.1,.12)$);
   \end{tikzpicture}}
\]
where the two free legs of the empty slot \emph{cross} on their way out:
the crossing is exactly the transpose that turns the adjugate's $X^{-T}$ into $X^{-1}$.
...
\[
   ...
   \begin{tikzpicture}[baseline=-.25em, inner sep=1pt]
      \node[copydot] (d1) at (0,0) {};
      \node (G) at (.7,0) {$G$};
      \node[copydot] (d2) at (1.4,0) {};
      \draw (d1) -- (G) -- (d2);
