...
\begin{align}
   ...
   \\
   \frac{d}{dx}\|x\|_2^2 &= \frac{d}{dx} x^T x = 2x
\end{align}
...
Let $F(t)=|I+t A|$ then
$F'(0) = \mathrm{Tr}(A)$ and
$F''(0) = \mathrm{Tr}(A)^2 - \mathrm{Tr}(A^2)$.
...
& +2 \mathbf{B X}\left(\mathbf{X}^T \mathbf{C X}\right)^{-1} \\
\frac{\partial}{\partial \mathbf{X}} \operatorname{Tr}\left[\left(\mathbf{A}+\mathbf{X}^T \mathbf{C X}\right)^{-1}\left(\mathbf{X}^T \mathbf{B X}\right)\right] & =-2 \mathbf{C X}\left(\mathbf{A}+\mathbf{X}^T \mathbf{C X}\right)^{-1} \mathbf{X}^T \mathbf{B X}\left(\mathbf{A}+\mathbf{X}^T \mathbf{C X}\right)^{-1} \\
