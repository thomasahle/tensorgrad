...
\begin{tabular}[h]{cccc}
   ...
   & \begin{tikzpicture}[baseline=-.25em]
      ...
      \node[inner sep=1pt, copydot] (d) at (1.25,0) {};
      \draw (u) -- (d);
      \draw (v) -- (d);
...
(1) The determinant function, $\det : \mathbb R^{n\times n} \to \mathbb R$ is a scalar function, and we can apply it to a batch of matrices by \tikz[baseline=(det.base), inner sep=2pt, every node/.append style={anchor=base}]{
      ...
      \draw (X) edge[->, bend left] node[midway, above, font=\tiny] {n} (det);
...
% 
  ...
  If the element-wise power function, $\mathrm{pow}_n : \mathbb R \to \mathbb R$
      is applied twice to the same vector, the factors simplify under the Hadamard product:
      \[
...
\[
   ...
   \begin{tikzpicture}[baseline=-1em, every node/.append style={anchor=base}]
      ...
      \node[right=1em of x2, inner sep=1pt, copydot] (s) {};
...
$(\tikz[baseline=(ex1.base), every node/.append style={anchor=base}]{
   ...
   \draw (ex1) edge[<-, densely dotted] (x1);
   \draw (x1) -- ++(.5,0);
}) \in \R^d$ is the element-wise exponential function.
If we contract it with $\tikz[baseline=(s.base)] {
   \node[inner sep=1pt, copydot] (s) {};
   \draw (s) -- ++(-.4,0);
...
One situation where things may get a bit confusing is when the output tensor is contracted with the broadcasting edges of an input tensor.
But as long as we remember to contract the function edges first, things work out.
For example, for a function $f : \R^n \to \R^m$ and a matrix $x \in \R^{m \times n}$:
\[
...
For example, if $f$ was $\sin-$, then $f'$ would be $\cos-$.
...
\[
H_{f\circ v}(x) = Dv(x)^T \cdot D^2f(v(x)) \cdot Dv(x) + \sum_{k=1}^m \frac{\partial f}{\partial v_k}(v(x)) \frac{\partial^2 v_k}{\partial x \partial x^T}(x).
\]
...
In the case of the determinant, the first derivative is the classical
\[
   \frac{\partial \det(X)}{\partial X} = \det(X)\,(X^{-1})^T.
\]
One may continue taking derivatives, but the pattern is not simply $\det(X)(X^{-1})^{\otimes k}$: each derivative introduces signs.
For $k=2$,
\[
   \frac{\partial^2 \det(X)}{\partial X_{ij}\,\partial X_{kl}}
   = \det(X)\left(X^{-1}_{ji} X^{-1}_{lk} - X^{-1}_{jk} X^{-1}_{li}\right),
\]
...
\subsection{Taylor}
For a sufficiently differentiable function $v: \R^d\to\R^m$ we can write the Taylor expansion:
\[
   \begin{tikzpicture}[baseline=-1em, inner sep=1pt]
      \node (n0) {$v$};
...
\begin{exercise}
   ...
   % Answer: Using the chain rule and elementwise differentiation, with $z = Wx+c$ one obtains
   % $\nabla_W KL = \frac{1}{2}\Big(e^{z} + 2z - 1\Big)x^T$, where the exponential is applied elementwise.
   Represent the KL-divergence term for a Variational Autoencoder (VAE) as
   \[
      KL(\mu,\sigma) = -\frac{1}{2}\sum_i\Big(1 + \log\sigma_i^2 - \mu_i^2 - \sigma_i^2\Big),
   \]
...
\begin{exercise}
   ...
   Let $A\in\mathbb{R}^{p\times m}$, $X\in\mathbb{R}^{m\times n}$, and $B\in\mathbb{R}^{q\times n}$. Prove the identity
   \[
      \operatorname{vec}(A\,X\,B^T) = (B \otimes A)\,\operatorname{vec}(X).
   \]
...
\begin{exercise}
   ...
   % Answer: For any unit eigenvector $u$ of the largest eigenvalue, $\phi(Y+\Delta) \ge u^T(Y+\Delta)u = \phi(Y) + \Tr(u u^T \Delta)$, so $G = u u^T$ is a subgradient.
   % The full subdifferential is the convex hull $\{\sum_i c_i u_i u_i^T : c_i \ge 0, \sum_i c_i = 1\}$; since it contains more than one point when $k>1$, no unique gradient exists.
   Let $Y\in\mathbb{R}^{n\times n}$ be a symmetric matrix, and define $\phi(Y)$ to be its largest eigenvalue. If this eigenvalue has multiplicity $k>1$, the function $\phi$ is not differentiable at $Y$. Show that for any unit eigenvector $u$ corresponding to the largest eigenvalue, the matrix
   \[
      G = u\,u^T
   \]
   is a valid subgradient of $\phi$ at $Y$. More generally, show that any convex combination $\sum_{i=1}^k c_i\, u_i u_i^T$, where $\{u_i\}_{i=1}^k$ is an orthonormal basis for the eigenspace corresponding to the largest eigenvalue, $c_i \ge 0$ and $\sum_{i=1}^k c_i = 1$, is a valid subgradient. Explain the challenges that arise in defining a unique gradient in this setting.
\end{exercise}
