...
Typically the main reason to do this is as a kludge for dealing with bad general notation for tensors.
...
\draw (c0) -- ++(-.5, .5) node[midway, above right, font=\tiny] {j};
\draw (c0) -- ++(-.5, -.5) node[midway, above left, font=\tiny] {i};
...
Which edge goes on top?
Throughout the book we use the convention that \emph{the top edge of a bundle is its most significant part}:
reading a bundle top to bottom matches reading the classical expression left to right,
the way we write numbers with the most significant digit first.
In the definition above, the bottom edge $i$ is the fast index and the top edge $j$ selects the block.
For the Kronecker products below, this means $A \otimes B$ is drawn with $A$ on top.
And since $\mathrm{vec}(X)$ stacks the \emph{columns} of $X$,
the column index is the most significant part of its bundle:
when $X$ is drawn to the left of a flattening triangle,
its column edge takes the top port and its row edge wraps under into the bottom port.
This convention also agrees with the wire order of the FFT diagrams in the Special Tensors chapter,
where the top wire carries the most significant bit.
One caveat: bundles behave like ribbons,
so a double wire that turns $180^\circ$ around comes back with its wires in reversed order;
when we intend the order to be preserved we mark a compensating \emph{twist} --- see the transpose rule (510) below.
...
\[
   ...
   \begin{tikzpicture}[baseline=-.25em]
      \node (X) at (.5,0) {X};
      \node[triangle] (c0) at (1.5,0) {};
      \draw (X) edge[bend left] node[midway, above, font=\tiny] {j} (c0);
      \draw (X) edge[bend right] node[midway, below, font=\tiny] {i} (c0);
      \draw[double] (c0) -- ++(.5, 0) node[midway, above, font=\tiny] {k};
   \end{tikzpicture}
   ,
\]
where the row index $i$ of $X$ takes the fast, bottom port and the column index $j$ the top.
...
%\begin{align}
   ...
   \begin{tikzpicture}[baseline=-.25em, inner sep=0pt]
      ...
      \node[copydot] (Fab0) at (.5,.25) {};
      \node (A) at (1,.333) {};
      \node (B) at (1,.166) {};
      \node[copydot] (Fcd0) at (.5,-.25) {};
      \node (C) at (1,-.166) {};
      \node (D) at (1,-.333) {};
      \draw[double] (F0) -- ++(-.4,0);
      \draw (F0) -- (Fab0);
...
%\end{align}
   ...
   \quad&=\quad
   \begin{tikzpicture}[baseline=-.5em, inner sep=0pt]
      \node[copydot] (b1) at (.4,.1) {};
      \node[copydot] (b2) at (.6,-.1) {};
      \draw (b1) -- (0, .1);
      \draw (b2) -- (0, -.1);
      \draw (b1) -- (1, .1);
      \draw (b2) -- (1, -.1);
...
The Kronecker product of an $m\times n$ matrix $A$ and a $p \times q$ matrix $B$, is an $mp \times nq$ matrix, $A \otimes B$ defined as
\[
...
$(A\otimes B)_{p(i-1)+k,\, q(j-1)+l} = A_{ij} B_{kl}$, but it's pretty hard to read.
...
\begin{walign}
   ...
   (A \otimes B)^T &= A^T \otimes B^T
      ...
      \begin{tikzpicture}[baseline=-.25em]
         ...
         \draw (F0) -- (A) -- (F1);
         \draw (F0) -- (B) -- (F1);
         \draw[twist] (F0) .. controls (-1,0) and (.6,1) .. (2,0);
         \draw[twist] (F1) .. controls (2.2,0) and (.6,-1) .. (-.8,0);
      \end{tikzpicture}
                   &=
      \begin{tikzpicture}[baseline=-.25em]
 ...
 \\&&&=
   ...
   \hspace{-1em}
\end{walign}
Note the small \emph{twists} on the wrapped bundles in (510).
A double wire is genuinely two parallel wires,
and dragging a bundle through a $180^\circ$ turn reverses their order on the page.
The twist --- the two wires crossing once --- compensates,
so the dangling bundle again reads $A$ before $B$.
Without the twists, the left-hand side of (510) would be the \emph{bit-reversed} transpose $B^T \otimes A^T$,
a genuinely different matrix!
For the same reason, rotating a flattened block $180^\circ$ is \emph{not} its transpose.
The trace loop in (515), on the other hand, needs no twist:
it turns a full $360^\circ$, so the two reversals cancel.
As a bonus, the twist on a straight bundle is a useful tensor in its own right:
it is exactly the \emph{commutation matrix} $K$,
satisfying $K(x \otimes y) = y \otimes x$ and $K\,\mathrm{vec}(X) = \mathrm{vec}(X^T)$.
...
\begin{walign}
   ...
   \begin{tikzpicture}[baseline=-.25em, inner sep=1pt]
      ...
      \node[copydot] (b1) at (1.9,.25) {};
      \node[copydot] (b2) at (1.9,-.25) {};
...
%    \begin{tikzpicture}[baseline=-.25em]
%       \node[triangle, rotate=180] (F0) at (0,0) {};
%       \node[copydot] (b1) at (.4,.25) {};
%       \node[copydot] (b2) at (.6,-.25) {};
...
\begin{walign}
   ...
   \begin{tikzpicture}[baseline=-.25em]
      ...
      % (.5,-.75) -- (3.5,-.75) node[midway,yshift=-3em]{$\mathrm{vec}(X)^T(B \otimes A)$};
   \end{tikzpicture}
   \\
   \tag{521}
...
\end{tikzpicture}
   ...
   \\
   \tag{524}
   a^T X B X^T c &= \mathrm{vec}(X)^T (B\otimes ac^T) \mathrm{vec}(X)
                 &
...
\end{center}
% TODO: re-verify the M1/M2/b factor order against figures/kron.pdf_tex under the top=most-significant convention
Can be written as a simple vector-matrix-matrix-vector product, $a^T M_1 M_2 b$,
...
=
   \begin{tikzpicture}[baseline=-.25em]
      ...
      \draw (c1) -- (A) -- (c3);
      \draw (c2) -- (B) -- (c4);
      \draw (c1) edge[bend left] (c3);
      \draw (c2) edge[bend right] (c4);
      \draw[double] (c3) -- ++(.4,0);
      \draw[double] (c4) -- ++(.4,0);
...
\[
   ...
   =
   \begin{tikzpicture}[baseline=-.25em]
      \node[copydot] (c0) at (0,0) {};
      \node (A) at (.6,.25) {A};
      \node (B) at (.6,-.25) {B};
      \node[copydot] (c1) at (1.2,0) {};
      \draw (c0) -- (A) -- (c1);
      \draw (c0) -- (B) -- (c1);
      \draw (c0) -- ++(-.4,0);
      \draw (c1) -- ++(.4,0);
...
\begin{walign}
   ...
   \begin{tikzpicture}[baseline=-.25em, inner sep=1pt]
      \node (A) at (0,0) {$A^T$};
      \node[copydot] (c0) at (.7,0) {};
      \node (B) at (1.3,0) {B};
      \node[copydot] (c1) at (1.9,0) {};
      \draw (A) -- (c0) -- (B) -- (c1);
   ...
   \begin{tikzpicture}[baseline=-.25em, inner sep=1pt]
      \node[triangle, rotate=180] (F0) at (0,0) {};
      \node[copydot] (Fab0) at (.5,.5) {};
      \node (A) at (1,.75) {A};
      \node (B) at (1,.25) {B};
      \node[copydot] (Fab1) at (1.5,.5) {};
      \node[copydot] (Fcd0) at (.5,-.5) {};
      \node (C) at (1,-.25) {C};
      \node (D) at (1,-.75) {D};
      \node[copydot] (Fcd1) at (1.5,-.5) {};
      \node[triangle] (F1) at (2,0) {};
      \draw[double] (F0) -- ++(-.4,0);
      \draw[double] (F1) -- ++(.4,0);
      \draw (F0) -- (Fab0);
...
Indeed, $(A \ast B)^T = A^T \bullet B^T$ and $(A \bullet B)^T = A^T \ast B^T$.
...
\begin{walign}
   ...
   \end{tikzpicture}
   \\
   (A x) \circ (B y) &= (A \bullet B) (x \otimes y)
                     &
   \begin{tikzpicture}[baseline=-.25em]
      \node[copydot] (c0) at (0,0) {};
      \draw (c0) -- ++(-.4,0);
...
An easy way to do this is taking the outer product with a vector that selects the location in the new axis, and then add them together.
...
$\mathrm{stack}_i(A, B) = A\mid B$ and $\mathrm{stack}_j(A, B) = \frac{A}{B}$.
...
\begin{exercise}
   ...
   \]
   with respect to $X$, where $G \in \mathbb{R}^{n^2 \times n^2}$,
   and $A$ and $X$ are $\mathbb R^{n \times n}$ matrices.

   Write the result in terms of $\mathrm{vec}(A)$ and a reshaped version of $G$.
\end{exercise}
