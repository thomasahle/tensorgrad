...
\newcommand{\drawPartitionAtAngle}[5][]{%
  ...
  \begin{scope}[shift={#2}]%
    ...
    \def\partitionDrawStyle{#1}%
    \def\partitionShowK{0}%
    \if\relax\detokenize{#1}\relax
      % no-op
    \else
      \def\karg{k}%
      \ifx\partitionDrawStyle\karg
        \def\partitionDrawStyle{}%
        \def\partitionShowK{1}%
      \fi
    \fi
    ...
    \foreach \offset in {42, 62, 82} {%
        ...
        \foreach [count=\blockIndex from 0] \block in #5 {%
            ...
            \foreach \elem in \block {%
                \pgfmathsetmacro{\elemAngle}{mod(30+(\elem - 1)*360/\globaln, 360)}%
    ...
    \foreach [count=\i from 0] \block in #5 {%
      ...
      \drawBlock[\partitionDrawStyle]{\currentAngle:\hubRadius}{\block}{\partitionShowK}%
    }%
  \end{scope}%
}
\newcommand{\drawBlock}[4][]{%
    ...
    \foreach \elem in #3 {%
        ...
        \draw[thin,#1] (hub) -- (node\elem);%
    }%
    \ifnum#4=0
        \node[inner sep=1pt, circle, fill, minimum size=2pt,#1] at (hub) {};%
    \else
