...
class CodegenContext:
    ...
    def _emit_function(self, t: Function) -> str:
        ...
        if isinstance(signature, F._PowerFunction):
            ...
            assert not t.orig_out, "argmax should have no output dims"
            edges = [e for e in t.inputs[0].edges if e != signature.dim]
        else:
        ...
        perm = [edges.index(t.orig_out.get(e, e)) for e in t.edges]
        if perm != list(range(t.order)):
            self.emit(f"{var_name} = {var_name}.permute({', '.join(map(str, perm))})  # {', '.join(t.edges)}")
        #shape = f"({', '.join(t.orig_out.get(e,e) for e in t.edges)},)" if t.edges else "()"
        #self.emit(f"assert {var_name}.shape == {shape}, ({var_name}.shape, {shape})")
        return var_name


    def _emit_derivative(self, t: Derivative) -> str:
        var_name = self.fresh_name("deriv_")
        self.emit(f"# Derivative wrt {t.x} => zero for now.")
        shape_dims = [self.declare_dimension(t.shape[e]) for e in t.edges]
        names_list = [f"'{e}'" for e in t.edges]
        self.emit(f"{var_name} = torch.zeros([{','.join(shape_dims)}], names=({','.join(names_list)}))")
        return var_name
...
"""
    ...
    def forward_with_values(values: dict[Variable, torch.Tensor], shapes: dict[sympy.Symbol, int]):
        ...
        call_args = (
            [s.name for s in all_symbols_sorted]
        ...
        return {tensor: output.refine_names(*tensor.edges)
