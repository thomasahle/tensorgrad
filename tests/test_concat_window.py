...
def test_window_slices():
    ...
    y = Variable("y", b=b, s=m)
    cat = F.concat(x, y, dim="s", size=k)
    w = cat @ F.window(s=k, t=j, start=1)  # cat[1:6], output edge named t
...
def test_window_zero_pads_when_output_larger():
    x = Variable("x", b=b, s=n)
    emb = x @ F.window(s=n, big=k)  # embed 3 into 7, zeros beyond
    vals = _vals()
...
def test_window_shift_pairing():
    ...
    a = x @ F.window(s=k, t=j)
    c = x @ F.window(s=k, t=j, start=1)
